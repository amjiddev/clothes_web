...
@section('title', 'NEW IN - Latest Arrivals')
...
<nav aria-label="breadcrumb" style="background: #f8f9fa; padding: 15px 0; border-bottom: 1px solid #e0e0e0;">
    <div class="container">
        <ol class="breadcrumb mb-0" style="background: transparent; padding: 0;">
            ...
            <li class="breadcrumb-item active" aria-current="page" style="color: var(--accent-gold); font-weight: 600;">NEW IN</li>
        </ol>
    </div>
</nav>
...
<section class="section-padding" style="background: white; padding: 60px 0;">
    <div class="container">
        ...
        <div class="row mb-5 align-items-center" style="border-bottom: 2px solid var(--accent-gold); padding-bottom: 20px;">
            <div class="col-md-6">
                <p class="mb-0" style="color: var(--primary-dark); font-weight: 500; font-size: 1.1rem;">
                    Showing <strong>{{ $products->count() }}</strong> of <strong>{{ $products->total() }}</strong> new arrivals
                </p>
            </div>
            <div class="col-md-6">
                <form method="GET" action="{{ route('new-in') }}" class="d-flex justify-content-end align-items-center" style="gap: 12px;">
                    <!-- Search Bar -->
                    ...
                    <select name="sort" class="form-select" style="width: auto; border-color: var(--accent-gold); font-size: 0.9rem; flex-shrink: 0;" onchange="this.form.submit()">
                        <option value="latest" {{ request('sort') == 'latest' ? 'selected' : '' }}>Sort: Latest First</option>
                ...
                <div class="product-card" style="background: white; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1); transition: all 0.3s ease;">
                    ...
                    <div style="position: relative; width: 100%; height: 320px; background: #e9e9e9; overflow: hidden;">
                        ...
                        @if($product->hasDiscount())
                            <span style="position: absolute; top: 15px; right: 15px; background: var(--accent-gold); color: var(--primary-dark); padding: 8px 15px; border-radius: 4px; font-size: 0.85rem; font-weight: 700; z-index: 5;">
        ...
        <div class="text-center py-5">
            ...
            <p class="text-muted" style="font-size: 1.1rem;">No new arrivals at the moment. Check back soon!</p>
...
function clearSearchText() {
    ...
    searchInput.focus();
    // Redirect to new-in page to show all new arrivals
    window.location.href = "{{ route('new-in') }}";
}
